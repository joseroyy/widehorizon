...
const MergedPlanes = ({ beamWidth, beamNumber, beamHeight, speed, noiseIntensity, scale }) => {
  ...
  const geometry = useMemo(() => 
    createStackedPlanesBufferGeometry(beamNumber, beamWidth, beamHeight, 0, 50), 
    [beamNumber, beamWidth, beamHeight]
  );
  
  useFrame((state, delta) => {
    uniforms.current.time.value += 0.05 * delta;
  });

  const onBeforeCompile = (shader) => {
    shader.uniforms.time = uniforms.current.time;
    shader.uniforms.uSpeed = uniforms.current.uSpeed;
    shader.uniforms.uNoiseIntensity = uniforms.current.uNoiseIntensity;
    ...
    shader.vertexShader = shader.vertexShader.replace(
      ...
      `#include <beginnormal_vertex>
      objectNormal = getNormal(position, uv);`
    );

    shader.vertexShader = shader.vertexShader.replace(
      '#include <begin_vertex>',
      `#include <begin_vertex>
      transformed.z += getPos(position, uv);`
    );
  ...
  return (
    <mesh ref={meshRef} geometry={geometry}>
      <meshPhysicalMaterial 
        ...
        customProgramCacheKey={() => 'beams-material'}
      />
    </mesh>
  );
};
...
const Beams = ({
  ...
  return (
    <div ref={containerRef} style={{ width: '100%', height: '100%', background: '#000', position: 'relative' }}>
      {isVisible && (
        <Canvas dpr={[1, 1]} gl={{ powerPreference: 'high-performance' }} style={{ position: 'absolute', inset: 0 }}>
          ...
          <group rotation={[0, 0, THREE.MathUtils.degToRad(rotation)]}>
            <MergedPlanes 
              ...
              scale={scale} 
            />
            <directionalLight color={lightColor} position={[0, 3, 10]} intensity={2} />
